---
title: Static race
excerpt: A static serve race between node and rust
date: 2024-03-22
hero: cover.png
author: Luke Celitan
appDescription: A static serve
category: Post
tech:
  - Rust
  - Nodejs
...
significantly impact the performance and efficiency of a website, and there is
no other backbone of the internet like the static server. A simple worker
checking the files on the drive and serving it back to the user.
...
page (200kb) and a single image (12.7mb). All applications ran on the same
machine, boasting 2GB of RAM and SSD storage, and were tested sequentially to
ensure fairness. Security was also a priority, with all connections made through
SSL, thanks to the host provider's network overlay.

This setup provides a glimpse into the real-world performance of these web
frameworks, offering valuable insights for developers and enthusiasts alike.
Whether you're building a personal project or scaling an enterprise application,
understanding the nuances of each server's capabilities is key to optimizing
your web presence and your server costs.
...
This is the test picture we will be stressing my hosting with.
...
Actix, a web framework written in Rust, has been turning heads with its
impressive HTML return times. It recently clocked in at a blistering 34.154ms,
showcasing the efficiency of Rust's design for concurrent web services. Nginx,
the stalwart of high-performance web servers, is hot on Actix's heels with a
return time of 34.349ms. This close competition highlights the advancements in
web server technology and the fine margins that separate the top performers.
...
it up, for 1000 requests most of them were delivered better than the competition
in both html and image file**
...
Express simplifies the server creation process for Node.js, allowing developers
to write less code while achieving more functionality. It provides a powerful
routing API, middleware support, and a suite of HTTP utility methods, making it
an ideal choice for building RESTful APIs.
...
Each framework has its strengths and weaknesses, and the choice often depends on
the specific needs of the project. For instance, if performance is the top
priority, Actix or Nginx might be the go-to options. However, for those looking
for a more straightforward development process with plenty of community support,
the good old Express could be still a valid choice.
