...
const WordEmbeddingCanvas = () => {
  ...
  useEffect(() => {
    ...
    const createArrow = (direction, color, length = 1) => {
      ...
      const arrowHelper = new window.THREE.ArrowHelper(dir, origin, length, color, length * 0.2, length * 0.1);
      return arrowHelper;
    };
    ...
    Object.entries(vectors).forEach(([label, { vec, color }]) => {
      ...
      const labelPosition = new window.THREE.Vector3(...vec).multiplyScalar(1.1);
