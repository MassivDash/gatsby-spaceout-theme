...
function Layout({ children, location, setNavigatorPosition }: LayoutProps) {
  ...
  const isMenuItem = allSite.edges[0].node.siteMetadata.menuLinks.some(item => `/${item.slug}` === location.pathname);
  console.log(isHomepage, location, isMenuItem, allSite.edges[0].node.siteMetadata.menuLinks)
